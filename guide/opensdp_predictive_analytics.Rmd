...
This guide will provide you with a baseline model to get you started and
introduce you to the main concepts of predictive analytics. From there, you will
learn techniques needed to interpret, understand, and iteratively improve the
model.
...
available carefully. You can learn more about the importance of data preparation
by reading this guide to EWS development.

This guide demonstrates the techniques of predictive analytics using a synthetic
dataset. The data in this guide were generated using the OpenSDP synthetic data
...
relationships you would find in a real-world dataset. In particular, there is
...
about the tradeoffs between transparency and accuracy.

In this guide, we will start from a dataset that has been prepared, but if you
are applying this to work in your organization, it's good to consider a few
questions about the prediction task before you start fitting models. First off,
take time to think about variables, time, and datasets. The sooner in a
student's academic trajectory you can make a prediction, the sooner you can
intervene--but the less accurate your predictions, and hence your intervention
targeting, is likely to be. What data, and specifically which variables, do you
have available to make predictions? What outcome are you trying to predict?
...
graduation status (late graduate, on time graduate, dropout, transferred out,
disappeared). For this guide you are being given the entire set of data so
...
Montucky. This dataset includes simulated data for multiple cohorts of 7th graders along
...
high school attended, and high school completion. For additional details on the
data elements that makeup the synthetic data, there is a codebook available
...
Model comparison is central to developing good predictive models. The dataset
...
for graduating. This is a common approach and predicted probabilities can be
informative on their own. However, many of the best methods for understanding
model accuracy for binary classification require us to compare a binary prediction
(will graduate, will not graduate) to the an observed binary outcome (did graduate,
did not graduate).

So, we need a way to convert this into a classification measure.
...
matrix). This matrix allows you to calculate at a glance a number of different
measures of accuracy - you can find great explanations of these metrics
on the
[Wikipedia page for confusion matrices](https://en.wikipedia.org/wiki/Confusion_matrix).


Now, let's turn to identifying the predictors available for building an
alternative model. Let's examine the student academic performance and behavioral
variables that you can use as predictors. These are mostly numerical variables,
so you should use the summary, histogram, and table commands to explore them.
Here's some syntax for examining 7th grade math scores. You can replicate and
edit it to examine other potential predictors and their distributions by
different subgroups.
...
```{r}
# TODO: Change this, should not be a correlation
...
regression of ontime graduation on seventh grade math scores.
...
Even before you use the predict command, you can use the logistic output to learn
something about the relationship between the predictor and the outcome variable.
The Pseudo $R^{2}$ (read: psuedo R-squared) is a proxy for the share of variation in the
...
logistic model. You can use the function `logit_rsquared()` in the `R/functions.R`
file included with this guide to calculate this for your model.

```{r}
logit_rsquared(math_model)
```

We want to increase the psuedo $R^{2}$ using the data we have, so we should
...
This metric allows you to explicitly weigh the tradeoff in identifying a higher
...
section will review a few techniques that I have found helful when doing 
...
combination of values of predictors that divde the outcome well between the 
two groups. 
...
we need the data from Bowers et. al. and a dataframe of 
